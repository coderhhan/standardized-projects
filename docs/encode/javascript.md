...
author:
  name: coderhhan
  link: https://github.com/coderhhan/standardized-projects
---
...
![javascript style](./img/javascript/JavaScript.svg)
...
- 1.3.2.【强制】对于逗号分隔的多行结构，最后一个逗号不加。`eslint`: [comma-dangle](https://eslint.org/docs/rules/comma-dangle)
...
- 1.7.1.【推荐】单行最大字符数：100。`eslint`: [max-len](https://eslint.org/docs/rules/max-len)
  ...
  // bad
  const foo =
    jsonData &&jsonData.foo && jsonData.foo.bar && jsonData.foo.bar.baz &&jsonData.foo.bar.baz.quux &&jsonData.foo.bar.baz.quux.xyzzy;
...
- 2.8.6.【强制】混合使用多种操作符时，用小括号包裹分组。`eslint`: [no-mixed-operators](https://eslint.org/docs/rules/no-mixed-operators)
  ...
  // good - 四则运算可以不用小括号包裹
  const bar = a + (b / c) * d;
  const bar2 = 1 + (2 / 1) * 2;//5
  const bar3 = 1 +2 / 1 * 2; //5
  ```
